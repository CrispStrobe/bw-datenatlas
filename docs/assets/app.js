...
const state={layer:'district_population',selected:{type:'state',id:'08'},variant:'migration_background',origin:'de_origins',allOrigins:false,areaPage:0,researchPage:0,researchRows:null,onlyLandtag:false,flowArea:'countries',compositionDetail:false,flowRange:'years',instOrganisation:'',instSource:'',zoom:{x:0,y:0,w:760,h:700}};
...
const layers={
 ...
 institutions:{title:'Islamische und alevitische Einrichtungen',badge:'Selbstveröffentlicht',date:'Eigene Verzeichnisse der Verbände und OpenStreetMap · Stand 2026',source:'bamf_fb55',unit:'points',note:'Punkte sind Einrichtungen, keine Bevölkerungszahlen. Aufgenommen ist, was öffentlich bekannt und öffentlich belegt ist: Jeder Punkt nennt die Seite, auf der die Anschrift steht, und verlinkt sie. Quellen sind die Verzeichnisse, die die Verbände selbst führen, Vereinsverzeichnisse der Städte, OpenStreetMap und einzeln belegte Fundstellen. Eine Verbandszugehörigkeit wird nur genannt, wenn eine Quelle sie selbst behauptet; sonst steht die Einrichtung ohne Verband. Keine personenbezogenen Angaben. Aus Einrichtungen lässt sich keine Zahl von Gläubigen ableiten.'},
...
function renderMap(){const l=layers[state.layer];$('map-title').textContent=l.title;$('map-period').textContent=l.date;$('map-badge').textContent=l.badge;$('map-badge').className='pill'+(isEstimate()?' warning':'');$('map-note').textContent=l.note;$('map-svg-title').textContent=l.title;$('map-svg-desc').textContent=l.date+'. '+l.note;renderLegend();$('map-unavailable').hidden=!!G;$('map').hidden=!G;$('export-map').disabled=!G;['zoom-in','zoom-out','zoom-reset'].forEach(id=>$(id).disabled=!G);if(!G)return;
 const municipalLayer=state.layer==='municipality_population'||state.layer==='religion_estimate_municipal'||state.layer==='muni_under25';const pointLayer=state.layer==='institutions';
...
function renderContext(){renderComposition();renderBwNationalities();renderResidence();renderNaturalisations();renderAreaFlows();renderFlows();}
